...
//    private static String EF_CACHE = "\\eigen.cache";
   ...
   // private static final String RECON_DIR = "‪E:\\Recons_09_May_2016";
  ...
  // -------------------- image files input -------------------------------
    public static BufferedImage[] loadTrainingIms(ArrayList<String> fnms) /* load all the specified images, making sure they are grayscale and the 
     same dimensions */ {
        ...
        int i = 0;
        System.out.println("FileUtils 76\tLoading grayscale images from " + TRAINING_DIR + "...");
        for (String fnm : fnms) {
            try {
                im = ImageIO.read(new File(fnm));
                System.out.println("  rrrr " + fnm);  // reading
                ims[i++] = ImageUtils.toScaledGray(ImageIO.read(new File(fnm)), 1.0);
                //ims[i++] = ImageUtils.toScaledGray(im, 1.0);    // make sure the images are gray
            } catch (Exception e) {
                System.out.println("FileUtils 84\tCould not read image from " + fnm + " error: " + e);
            }
        }
        System.out.println("FileUtils 87\tLoading done\n");
    ...
    public static BufferedImage loadImage(String imFnm) // return an image
    {
        ...
        try {
            ...
            System.out.println("ding ding fileutil line 106 Could not read image from " + imFnm + " error: " + e);
        }
        return image;
    }  // end of loadImage()
