...
public class BuildEigenFaces {
    ...
    public static void build(int numEFs) // create a FaceBundle for the specified number of eigenfaces, and store it
    {
        ...
        System.out.println("buildeigenfaces.build er shuru at line 35");

        ArrayList<String> fnms = FileUtils.getTrainingFnms();
        System.out.println("buildeigenfaces.build er fnms shesh at line 37");

        if (fnms != null) {    //modified by tapos  
            int numIms = fnms.size();

            System.out.println("numIms: " + numIms);
            ...
            numEFs = 15;
            System.out.println("buildEigenFaces at line 51,\tnumEFs is now: " + numEFs);
            FaceBundle bundle = makeBundle(fnms);
            System.out.println("sob thik thak at buildEigenfaces, at line 53");
            FileUtils.writeCache(bundle);
            System.out.println("reconstruct baad deya hoyeche");
    ...
    // also save each eigenface (eigenvector) as an image file
    {
        ...
        System.out.println("BuildEigenfaces line 82\tsortEigenfaces er aag porjonto thik ase");
